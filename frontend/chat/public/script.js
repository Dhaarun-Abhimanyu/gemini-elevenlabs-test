    ...
    if (intersects.length > 0) {
        ...
        if (hoveredObject !== hoveredSegment) {
            if (hoveredSegment) {
                ...
                const previousTextElement = document.getElementById(`text-${hoveredSegment.name}`);
    ...
    if (intersects.length > 0) {
        ...
        if (["1", "2", "3", "4", "5", "6"].includes(clickedObject.name)) {
            ...
            new TWEEN.Tween(camera.rotation)
                .to({ x: targetRotationX, y: camera.rotation.y, z: camera.rotation.z }, 1000) // 1 second rotation
                .easing(TWEEN.Easing.Quadratic.Out)
                .start();
        }
    }
